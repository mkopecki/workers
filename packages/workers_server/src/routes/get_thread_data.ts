...
export const get_thread_data: H = async (c) => {
  ...
  // retrieve data
  const threads_promise = db.select().from(threads_table).where(eq(threads_table.id, thread_id));
  ...
  const runs_promise = db.select().from(runs_table).where(eq(runs_table.thread_id, thread_id));
  ...
  const [threads, thread_states, messages, runs, extended_run_steps] = await Promise.all([
    threads_promise,
...
function group_by<T, K extends keyof T>(items: T[], key: K): { [group: string]: T[] } {
  return items.reduce(
    (acc, item) => {
